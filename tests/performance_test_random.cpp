                        ...
                        std::vector<typename Alloc::pointer>& ptr_vec)
{
    ...
    std::bernoulli_distribution coin_flip(
        0.5); // 50% chance for allocation or deallocation
    ...
    while (steps_used < MAX_STEP)
    {
        ...
        const bool do_free = must_alloc  ? false
                             : must_free ? true
                                         : coin_flip(rng);
        if (do_free)
        {
            ...
            {
                for (auto dest = ptr_vec.end() - BLOCK_SIZE;
                     dest != ptr_vec.end() - BLOCK_SIZE / 2; ++dest)
                {
                    auto rand_it =
                        ptr_vec.begin() + rng() % (ptr_vec.size() - BLOCK_SIZE);
                    std::swap(*dest, *rand_it);
                }
            }
            // Deallocate
            auto start_deallocate = std::chrono::steady_clock::now();
            for (auto it = ptr_vec.end() - BLOCK_SIZE; it != ptr_vec.end();
                 ++it)
            {
                T* ptr = *it;
        ...
        else
        {
            ...
            total_time += std::chrono::duration_cast<std::chrono::microseconds>(
                              end_reallocate - start_reallocate)
                              .count();
        }
        steps_used++;
    }
...
class StackAllocator
{
  ...
  private:
    ...
    std::stack<pointer, std::vector<pointer>>
        stack;                             // Stack to hold allocated pointers
...
{
    pool_time = run_allocator_benchmark("Pool Allocator", pool_allocator,
                                        pool_ptr_vector);
    default_time = run_allocator_benchmark(
        "Default Allocator", default_allocator, default_ptr_vector);
    stack_time = run_allocator_benchmark("Stack Allocator", stack_allocator,
                                         stack_ptr_vector);

    // Sanity check for test run time
    ASSERT_GT(pool_time, 0) << "Pool allocator time should be greater than 0";
    ASSERT_GT(default_time, 0)
        << "Default allocator time should be greater than 0";
    ASSERT_GT(stack_time, 0) << "Stack allocator time should be greater than 0";

    // Assert that vectors are empty after the test
    ASSERT_TRUE(pool_ptr_vector.empty())
        << "Pool allocator vector should be empty after test";
    ASSERT_TRUE(default_ptr_vector.empty())
        << "Default allocator vector should be empty after test";
    ASSERT_TRUE(stack_ptr_vector.empty())
        << "Stack allocator vector should be empty after test";

    // Assert that pool allocator is faster than default allocator
    ASSERT_LT(pool_time, default_time)
        << "Pool allocator should be faster than default allocator";

    // print a relative performance comparison table with the default allocator
    // as the baseline
    std::cout << "Performance Comparison:\n";

    std::cout << std::left << std::setw(12) << "Allocator" << std::right
              << std::setw(12) << "Time (us)" << std::setw(16) << "Relative (%)"
              << "\n"
              << std::fixed << std::setprecision(1);

    std::cout << std::left << std::setw(12) << "Default" << std::right
              << std::setw(12) << default_time << std::setw(16) << "100.0%"
              << "\n";
    std::cout << std::left << std::setw(12) << "Pool" << std::right
              << std::setw(12) << pool_time << std::setw(15) << std::fixed
              << std::setprecision(1)
              << (static_cast<long double>(pool_time) / default_time) * 100
              << "%" << "\n";
    std::cout << std::left << std::setw(12) << "Stack" << std::right
              << std::setw(12) << stack_time << std::setw(15) << std::fixed
              << std::setprecision(1)
