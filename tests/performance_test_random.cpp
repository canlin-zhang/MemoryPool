                        ...
                        std::vector<typename Alloc::pointer>& ptr_vec)
{
    ...
    total_time +=
        std::chrono::duration_cast<std::chrono::microseconds>(end_init - start_init).count();
    ...
    while (steps_used < MAX_STEP)
    {
        ...
        const bool do_free = must_alloc ? false : must_free ? true : coin_flip(rng);
        if (do_free)
        {
            ...
            total_time += std::chrono::duration_cast<std::chrono::microseconds>(end_deallocate -
                                                                                start_deallocate)
                              .count();
        }
        else
        {
            ...
            total_time += std::chrono::duration_cast<std::chrono::microseconds>(end_reallocate -
                                                                                start_reallocate)
                              .count();
        }
        steps_used++;
    ...
    total_time +=
        std::chrono::duration_cast<std::chrono::microseconds>(end_cleanup - start_cleanup).count();
...
TEST_F(PoolAllocatorTest, allocator_perf)
{
    ...
    default_time =
        run_allocator_benchmark("Default Allocator", default_allocator, default_ptr_vector);
    ...
    std::cout << std::left << std::setw(12) << "Allocator" << std::right << std::setw(12)
              << "Time (us)" << std::setw(16) << "Relative (%)" << "\n"
              << std::fixed << std::setprecision(1);

    std::cout << std::left << std::setw(12) << "Default" << std::right << std::setw(12)
              << default_time << std::setw(16) << "100.0%" << "\n";
    std::cout << std::left << std::setw(12) << "Pool" << std::right << std::setw(12) << pool_time
              << std::setw(15) << std::fixed << std::setprecision(1)
              << (static_cast<long double>(pool_time) / default_time) * 100 << "%" << "\n";
    std::cout << std::left << std::setw(12) << "Stack" << std::right << std::setw(12) << stack_time
              << std::setw(15) << std::fixed << std::setprecision(1)
